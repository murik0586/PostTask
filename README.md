...
--
_____
Личные сообщения в социальном сервисе.

Можно реализовать как сочту нужным. Несколько требований:


>TASK 1:Должны быть чаты.
  
  1. Success Можно создавать чаты.

  2. Success Удалять их, получать список имеющихся чатов.

  3. Success Можно создавать сообщения.
  4. Success Редактировать их
  5. Success Удалять. Для простоты — можно удалять и свои, и чужие.
  6. Success В каждом чате есть прочитанные и непрочитанные сообщения.


>TASK 2: Возможности для пользователя:

1. Success Видеть, сколько чатов не прочитано.
*В каждом из таких чатов есть хотя бы одно непрочитанное сообщение.*

2. Success Получить список чатов (например, service.getChats).
3. Success Получить список последних сообщений из чатов (можно в виде списка строк). Если сообщений в чате нет (все были удалены), то пишется «нет сообщений».
4. Success Получить список сообщений из чата, указав:

ID собеседника;

Количество сообщений. 

Success После того как вызвана эта функция, все отданные сообщения автоматически считаются прочитанными.

5. Success Создать новое сообщение.

6. Success Удалить сообщение.
7. Success Создать чат. Чат создаётся, когда пользователю отправляется первое сообщение.
8. Success Удалить чат, т. е. целиком удалить всю переписку.
>Success Старайтесь использовать lambda-функции (их напишите сами) и extension-функции (есть в составе Iterable, Collection, List).

>Success Расчёт статистики старайтесь производить как цепочку вызовов lambda-функций. Попробуйте обойтись без for, while и do-while.

>Success у вас должен быть репозиторий на GitHub, в котором расположен ваш Gradle-проект. Автотесты также должны храниться в репозитории.

>Важно: авто-тесты должны быть, в том числе на исключения.
_____
ОПТИМИЗАЦИЯ ПОСЛЕДОВАТЕЛЬНОСТИ ChatService
--
_____
> ChatService Optimization
Оптимизировать проект с предыдущего ДЗ (ChatService) и переведите его на Sequence.

Оформите всё в виде Pull Request. 
